...
class DetectionBatchSingleTester {
public:
    explicit DetectionBatchSingleTester(float threshold) : error_threshold_(threshold) {}

    DetectionBatchSingleTester& TestPrepare(const uint32_t& num_classes, const bool& share_location,
                                            const float& nms_threshold, const int32_t& background_label_id,
    ...
    void TestRun(Dim4& output_dim, const float* output_expect, Status status = Status::SUCCESS) {
        ...
        DetectionBatchSingle _detection_batch_1(PrecisionType::FP32);

        EXPECT_EQ(_detection_batch_1.initialize(loc_dim_, conf_dim_, prior_dim_, output_dim, num_classes_, share_location_,
                  ...
                  Status::SUCCESS);
        EXPECT_EQ(_detection_batch_1.execute(input_tensor_loc, input_tensor_conf, input_tensor_prior, output_tensor),
                  status);
        EXPECT_EQ(_detection_batch_1.release(), Status::SUCCESS);
...
TEST(ENN_CPU_OP_UT_DetectionBatchSingle, detection_test_1) {
    ...
    DetectionBatchSingleTester(1e-5)
...
TEST(ENN_CPU_OP_UT_DetectionBatchSingle, detection_test_2) {
    const uint32_t num_classes = 2;
    const bool share_location = true;
    ...
    uint32_t batch = 1;
    Dim4 output_dim = {batch, 1, nms_top_k, 7};
    const float output_expect[] = {0, 1, 1.0, 0.107309, 0.107309, 0.422691, 0.422691,
                                   0, 1, 0.8, 0.592316, 0.107309, 0.877684, 0.422691};
