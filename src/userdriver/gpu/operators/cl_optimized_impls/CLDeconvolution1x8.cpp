...
CLDeconvolution1x8::CLDeconvolution1x8(const std::shared_ptr<CLRuntime> runtime,
    ...
    DEBUG_PRINT("CLDeconvolution1x8 is created");
    runtime_ = runtime;
    ...
    Status state = runtime_->setKernel(&kernel_trans_, "deconv_8x1_matrixTrans", precision_);
    CHECK_EXPR_NO_RETURN(Status::SUCCESS == state, "matrixTrans setKernel failure\n");
    state = runtime_->setKernel(&kernel_gemm_, "deconvGemm_1x8", precision_);
    ...
    uint32_t filter_align_height = alignTo(kernel.h * kernel.w * output_dim.c, 1);
    uint32_t filter_count = filter_align_width * filter_align_height;

    Dim4 filter_buffer_dim = {filter_count, 1, 1, 1};
    filter_buffer_ = std::make_shared<CLTensor>(runtime_,
                                                precision_,
                                                DataType::FLOAT,
                                                filter_buffer_dim,
                                                DataOrder::NCHW,
    ...
    uint32_t transinput_align_height = alignTo(input_dim.h * input_dim.w, 8);
    uint32_t transinput_align_width = alignTo(input_dim.c / group_, 8) * group_;
    uint32_t size_trans_input = input_dim.n * 1 * transinput_align_height * transinput_align_width;
...
Status CLDeconvolution1x8::execute(const std::shared_ptr<ITensor> input,
                                   std::shared_ptr<ITensor> output) {
    DEBUG_PRINT("CLDeconvolution1x8::execute() is called");
    ...
    // align 8 rows avoiding reading bottom BoundsException
    uint32_t filter_align_width = alignTo(input_dim.c / group_, 8);
    ...
    global_weight_trans[2] = filter_align_width;
    ...
    global_input_trans[2] = trans_width_align;
    ...
    size_t local_gemm[3] = {1, 4, 16};
    global_gemm[0] = group_;
    global_gemm[1] = alignTo(row_filter, 4);  // Each thread process 8 elements
    global_gemm[2] = alignTo(row_input, 16);
    for (uint32_t i = 0; i < input_dim.n; i++) {
...
Status CLDeconvolution1x8::release() {
    DEBUG_PRINT("CLDeconvolution1x8::release() is called");
    return Status::SUCCESS;
}
