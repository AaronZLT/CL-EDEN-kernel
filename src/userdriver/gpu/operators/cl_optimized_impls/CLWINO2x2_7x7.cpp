...
CLWINO2x2_7x7::CLWINO2x2_7x7(const std::shared_ptr<CLRuntime> runtime,
                             const PrecisionType &precision,
                             const Dim4 &input_dim,
                             const Dim4 &output_dim) :
    runtime_(runtime),
    precision_(precision), input_dim_(input_dim), output_dim_(output_dim) {
    ENN_DBG_PRINT("CLWINO2x2_7x7 is created");
...
Status CLWINO2x2_7x7::initialize(const Pad4 &padding,
                                 const Dim2 &stride,
    ...
    ENN_DBG_PRINT("CLWINO2x2_7x7::initialize() is called");
    ...
    Status state = runtime_->setKernel(&weight_tm_kernel_, "wino7x7weight_tm_nchw_opt", precision_);
    CHECK_EXPR_RETURN_FAILURE(Status::SUCCESS == state, "setKernel wino7x7weight_tm_nchw_opt failure!\n");

    state = runtime_->setKernel(&input_tm_kernel_, "wino7x7input_tm_nchw_opt", precision_);
    CHECK_EXPR_RETURN_FAILURE(Status::SUCCESS == state, "setKernel wino7x7input_tm_nchw_opt failed!\n");

    state = runtime_->setKernel(&dot_multiply_kernel_, "wino7x7dot_multiply_nchw_opt", precision_);
    CHECK_EXPR_RETURN_FAILURE(Status::SUCCESS == state, "setKernel wino7x7dot_multiply_nchw_opt failed!\n");
    if (!activation_info_.isEnabled()) {
        state = runtime_->setKernel(&output_tm_kernel_, "wino7x7output_tm_nchw_opt", precision_);
        CHECK_EXPR_RETURN_FAILURE(Status::SUCCESS == state, "setKernel wino7x7output_tm_nchw_opt failed!\n");
    } else {
        if (activate_info.activation() == ActivationInfo::ActivationType::RELU) {
            state = runtime_->setKernel(&output_tm_kernel_, "RELUwino7x7output_tm_nchw_opt", precision_);
            CHECK_EXPR_RETURN_FAILURE(Status::SUCCESS == state, "setKernel RELUwino7x7output_tm_nchw_opt failed!\n");
        } else {
            ERROR_PRINT("Non supported kernel size %dx%d in wino2x2_7x7 convolution\n", weight_dim_.h, weight_dim_.w);
    ...
    tile_size_ = 64;
    aligned_width_ = 2;
    wgrad_tile_height_ = (output_dim_.h + 1) / 2;
    wgrad_tile_width_ = (output_dim_.w + 1) / 2;

    wgrad_tile_width_ = alignTo(wgrad_tile_width_, aligned_width_);
    input_c_aligned_ = alignTo(input_dim_.c, aligned_inch_);
    output_c_aligned_ = alignTo(output_dim_.c, aligned_outch_);

    // four align
    wgrad_tile_total_ = wgrad_tile_height_ * wgrad_tile_width_;
    if (0 == wgrad_tile_total_ % 32) {
        coalescing_feature_height_ = 32;
    } else if (0 == wgrad_tile_total_ % 24) {
        coalescing_feature_height_ = 24;
    } else if (0 == wgrad_tile_total_ % 36) {
        coalescing_feature_height_ = 36;
    } else {
        coalescing_feature_height_ = 32;
    }
    ...
    input_tm_buffer_ = std::make_shared<CLTensor>(
        runtime_, precision_, DataType::HALF, input_tm_dim_, DataOrder::OTHER, 1.0, 0, BufferType::INTRA_SHARED);
    ...
    dot_buffer_ = std::make_shared<CLTensor>(
        runtime_, precision_, DataType::HALF, dot_dim_, DataOrder::OTHER, 1.0, 0, BufferType::INTRA_SHARED);
...
Status CLWINO2x2_7x7::execute(const std::shared_ptr<ITensor> input, std::shared_ptr<ITensor> output) {
    ENN_DBG_PRINT("CLWINO2x2_7x7::execute() is called");
    ...
    global[1] = 4;
    global[2] = input_c_aligned_ * input_dim.n;
    ...
    Status status = runtime_->setKernelArg(input_tm_kernel_.get(),
                                           ...
                                           wgrad_tile_width_ / 2,
                                           aligned_inch_);
    CHECK_EXPR_RETURN_FAILURE(Status::SUCCESS == status, "setKernelArg WINO7x7input_tm_nchw_opt failed\n");

    status = runtime_->enqueueKernel(input_tm_kernel_.get(), (cl_uint)3, global, local);
    CHECK_EXPR_RETURN_FAILURE(Status::SUCCESS == status, "enqueue WINO7x7input_tm_nchw_opt failed\n");

    // 2. dot
    global[0] = aligned_tiles_ / 4;
    global[1] = 64;
    global[2] = output_c_aligned_ / aligned_outch_ * output_dim.n;

    local[0] = coalescing_feature_height_ / 4;
    local[1] = 4;
    local[2] = output_c_aligned_ / aligned_outch_;
    if (local[2] * local[1] * local[0] >= 512) {
        local[1] = 2;
    }
    ...
    dst_n = input_dim.n;
    dst_c = output_c_aligned_ / aligned_outch_;
    dst_h = src_h;
    dst_w = src_w;
    ...
    status = runtime_->setKernelArg(dot_multiply_kernel_.get(),
                                    ...
                                    aligned_outch_,
                                    aligned_inch_);
    CHECK_EXPR_RETURN_FAILURE(Status::SUCCESS == status, "setKernelArg WINO7x7dot_multiply_nchw_opt failed\n");

    status = runtime_->enqueueKernel(dot_multiply_kernel_.get(), (cl_uint)3, global, local);
    CHECK_EXPR_RETURN_FAILURE(Status::SUCCESS == status, "enqueue WINO7x7dot_multiply_nchw_opt failed\n");

    // 3. output_tm
    global[0] = aligned_tiles_ / 2;
    global[1] = 2;
    global[2] = output_c_aligned_ * output_dim.n;
    ...
    status = runtime_->setKernelArg(output_tm_kernel_.get(),
                                    ...
                                    aligned_outch_,
                                    aligned_inch_);
    CHECK_EXPR_RETURN_FAILURE(Status::SUCCESS == status, "setKernelArg WINO7x7output_tm_nchw_opt failed\n");

    status = runtime_->enqueueKernel(output_tm_kernel_.get(), (cl_uint)3, global, local);
    CHECK_EXPR_RETURN_FAILURE(Status::SUCCESS == status, "enqueue WINO7x7output_tm_nchw_opt failed\n");

    return Status::SUCCESS;
}

Status CLWINO2x2_7x7::convertWeight() {
    ENN_DBG_PRINT("CLWINO2x2_7x7 convertWeight");
    if (androidNN_) {
    ...
    size_t local[2] = {16, 1};
    size_t global[2] = {(size_t)alignTo(weight_dim_.c, aligned_inch_), (size_t)alignTo(weight_dim_.n, aligned_outch_)};

    global[0] = alignTo(global[0], local[0]);
    global[1] = alignTo(global[1], local[1]);

    int weight_tm_size = tile_size_ * alignTo(weight_dim_.c, aligned_inch_) * alignTo(weight_dim_.n, aligned_outch_);
