...
CLDiv::CLDiv(const std::shared_ptr<CLRuntime> runtime, const PrecisionType &precision) :
    runtime_(runtime), precision_(precision) {
    ENN_DBG_PRINT("CLDiv is created\n");
...
Status CLDiv::initialize(const std::vector<std::shared_ptr<ITensor>> &input_tensors,
                         const std::vector<std::shared_ptr<ITensor>> &output_tensors,
                         const std::shared_ptr<Parameters> parameters) {
    ENN_DBG_PRINT("CLDiv::initialize() is called\n");
    ...
    parameters_ = std::static_pointer_cast<DivParameters>(parameters);
    ...
    if (input_tensor_0_->getDataType() == DataType::INT32) {
        status = runtime_->setKernel(&kernel_, "INT32div", precision_);
        CHECK_EXPR_RETURN_FAILURE(Status::SUCCESS == status, "setKernel INT32div failure\n");
    } else {
        status = runtime_->setKernel(&kernel_, "div", precision_);
        CHECK_EXPR_RETURN_FAILURE(Status::SUCCESS == status, "setKernel div failure\n");
    }
    ...
    if (parameters_->activation_info.isEnabled()) {
        ...
        activation_parameters->relu_parameters = std::make_shared<ReluParameters>();
...
Status CLDiv::execute() {
    ENN_DBG_PRINT("CLDiv::execute() is called\n");

    if (input_tensor_0_->getTotalSizeFromDims() == 0 || input_tensor_1_->getTotalSizeFromDims() == 0) {
        ENN_DBG_PRINT("CLDiv execute return for zero_sized input. input_0 size = %d, input_1 size = %d\n",
    ...
    status = runtime_->setKernelArg(
        kernel_.get(), input_broadcast_0_->getDataPtr(), input_broadcast_1_->getDataPtr(), output_tensor_->getDataPtr());
...
Status CLDiv::release() {
    ENN_DBG_PRINT("CLDiv::release() is called");
    return Status::SUCCESS;
}
