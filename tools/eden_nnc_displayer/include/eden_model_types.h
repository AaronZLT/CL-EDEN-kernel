...
#ifndef addr_t
typedef char* addr_t;         // for address type
#endif
...
static const char* const PRE_DSP_IV3_DIV_1M = "DSP_PRE_BGR_TO_BGRX_DIV_1M";

static const char* const SHARED_MEM = "Shared_Mem";
#define NO_SHARED_MEM_IDX (-1)
...
typedef enum {
  ...
  EDEN_MEM_TYPE_ION_NON_CACHEABLE = 2,
} EdenMemType;
...
typedef struct __EdenBufferInfo {
  ...
  EdenBufferLayout bufferLayout;      /*!< data layout of buffer (NCHW, NHWC, CUSTOM etc) */
...
typedef struct __EdenBufferQuantizeInfo {
  EDEN_QUANT_DATA_TYPE quantizeDataType;  /*!< quantization data type (INT8, INT16, INT32) */
...
typedef struct __ClipOptions {
    float min;
    float max;
} ClipOptions;

typedef struct __EluOptions {
    float alpha;
} EluOptions;
...
struct EdenOperation {
  ...
  size_t custom_size;
  EdenOperation() : opType(0), numOfInputs(0), inputOperandIndexes(nullptr),
                    numOfOutputs(0), outputOperandIndexes(nullptr),
                    hasOptions(false), isNCHW(false), delegateNHWC(false),
                    custom_buffer(nullptr), custom_size(0) {
    opName.name = nullptr;
    opName.length = 0;
  }
  ...
  ~EdenOperation() {
    ...
    if (inputOperandIndexes) {
      delete[] inputOperandIndexes;
      inputOperandIndexes = nullptr;
    }

    if (outputOperandIndexes) {
      delete[] outputOperandIndexes;
      outputOperandIndexes = nullptr;
    }

    if (custom_buffer) {
      delete[] custom_buffer;
      custom_buffer = nullptr;
    }
  }
};
#else
...
typedef struct __EdenNode {
  ...
  std::vector<struct __EdenNode*> forwardNodes;   /*!< neighbors for forward path */
