...
CLWINO6x6_3x3::CLWINO6x6_3x3(const std::shared_ptr<CLRuntime> runtime,
                             const PrecisionType &precision,
                             const Dim4 &input_dim,
                             const Dim4 &output_dim) :
    runtime_(runtime),
    precision_(precision), input_dim_(input_dim), output_dim_(output_dim) {
    ENN_DBG_PRINT("CLWINO6x6ConvolutionoOpt_4x16 is created");
...
Status CLWINO6x6_3x3::initialize(const Pad4 &padding,
                                 const Dim2 &stride,
    ...
    ENN_DBG_PRINT("CLWINO6x6_3x3::initialize() is called");
    ...
    Status state = runtime_->setKernel(&weight_tm_kernel_, "wino6x6_weight_tm_nchw_opt_4x16", precision_);
    CHECK_EXPR_RETURN_FAILURE(Status::SUCCESS == state, "setKernel wino6x6_weight_tm_nchw_opt_4x16 failure!\n");

    state = runtime_->setKernel(&input_tm_kernel_, "wino6x6_input_tm_nchw_opt_4x16", precision_);
    CHECK_EXPR_RETURN_FAILURE(Status::SUCCESS == state, "setKernel wino6x6_input_tm_nchw_opt_4x16 failed!\n");

    state = runtime_->setKernel(&dot_multiply_kernel_, "wino6x6_dot_multiply_nchw_opt_4x16", precision_);
    CHECK_EXPR_RETURN_FAILURE(Status::SUCCESS == state, "setKernel wino6x6_dot_multiply_nchw_opt_4x16 failed!\n");
    if (!activation_info_.isEnabled()) {
        state = runtime_->setKernel(&output_tm_kernel_, "wino6x6_output_tm_nchw_opt_4x16", precision_);
        CHECK_EXPR_RETURN_FAILURE(Status::SUCCESS == state, "setKernel wino6x6_output_tm_nchw_opt_4x16 failed!\n");
    } else {
        if (activate_info.activation() == ActivationInfo::ActivationType::RELU) {
            state = runtime_->setKernel(&output_tm_kernel_, "RELUwino6x6_output_tm_nchw_opt_4x16", precision_);
            CHECK_EXPR_RETURN_FAILURE(Status::SUCCESS == state, "setKernel RELUwino6x6_output_tm_nchw_opt_4x16 failed!\n");
        } else {
            ERROR_PRINT("Non supported kernel size %dx%d in wino6x6_3x3 convolution\n", weight_dim_.h, weight_dim_.w);
    ...
    wgrad_tile_height_ = (output_dim_.h + 5) / 6;
    wgrad_tile_width_ = (output_dim_.w + 5) / 6;
    ...
    input_tm_buffer_ = std::make_shared<CLTensor>(
        runtime_, PrecisionType::FP16, DataType::HALF, input_tm_dim_, DataOrder::OTHER, 1.0, 0, BufferType::DEDICATED);
    ...
    dot_buffer_ = std::make_shared<CLTensor>(
        runtime_, PrecisionType::FP16, DataType::HALF, dot_dim_, DataOrder::OTHER, 1.0, 0, BufferType::DEDICATED);
...
Status CLWINO6x6_3x3::execute(const std::shared_ptr<ITensor> input, std::shared_ptr<ITensor> output) {
    ENN_DBG_PRINT("CLWINO6x6_3x3::execute() is called");
    ...
    Status status = runtime_->setKernelArg(input_tm_kernel_.get(),
                                           ...
                                           aligned_inch_,
                                           aligned_pixel_);
    CHECK_EXPR_RETURN_FAILURE(Status::SUCCESS == status, "setKernelArg wino6x6_input_tm_nchw_opt_4x16 failed\n");

    status = runtime_->enqueueKernel(input_tm_kernel_.get(), (cl_uint)3, global, local);
    CHECK_EXPR_RETURN_FAILURE(Status::SUCCESS == status, "enqueue wino6x6_input_tm_nchw_opt_4x16 failed\n");
    ...
    status = runtime_->setKernelArg(dot_multiply_kernel_.get(),
                                    ...
                                    aligned_inch_,
                                    aligned_pixel_);
    CHECK_EXPR_RETURN_FAILURE(Status::SUCCESS == status, "setKernelArg wino6x6_dot_multiply_nchw_opt_4x16 failed\n");

    status = runtime_->enqueueKernel(dot_multiply_kernel_.get(), (cl_uint)3, global, local);
    CHECK_EXPR_RETURN_FAILURE(Status::SUCCESS == status, "enqueue wino6x6_dot_multiply_nchw_opt_4x16 failed\n");
    ...
    status = runtime_->setKernelArg(output_tm_kernel_.get(),
                                    ...
                                    aligned_inch_,
                                    aligned_pixel_);
    CHECK_EXPR_RETURN_FAILURE(Status::SUCCESS == status, "setKernelArg wino6x6_output_tm_nchw_opt_4x16 failed\n");

    status = runtime_->enqueueKernel(output_tm_kernel_.get(), (cl_uint)3, global, local);
    CHECK_EXPR_RETURN_FAILURE(Status::SUCCESS == status, "enqueue wino6x6_output_tm_nchw_opt_4x16 failed\n");

    return Status::SUCCESS;
}

Status CLWINO6x6_3x3::convertWeight() {
    ENN_DBG_PRINT("CLWINO6x6_3x3 convertWeight");
    ...
    weight_tm_buffer_ = std::make_shared<CLTensor>(runtime_, PrecisionType::FP16, DataType::HALF, weight_tm_buffer_dim);
