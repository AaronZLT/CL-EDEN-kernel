...
#define DATA_T uchar
ADD_SINGLE_KERNEL(relu1_INT8, (__global DATA_T *input, __global DATA_T *output, int min, int max) {
    Q_RELU_X(input, output, min, max)
})
#undef DATA_T

#define DATA_T char
ADD_SINGLE_KERNEL(SIGNEDrelu1_INT8, (__global DATA_T *input, __global DATA_T *output, int min, int max) {
    Q_RELU_X(input, output, min, max)
})
#undef DATA_T

// apply RELU1 to kernels' output
#define ACT_VEC_F(VEC_T, x) RELU1_VEC_F(VEC_T, x)
/********  FP16 KERNELS ********/
#define DATA_T half
...
ADD_SINGLE_KERNEL(relu1_FP16, (__global DATA_T *input,
...
ADD_SINGLE_KERNEL(relu1_FP32, (__global DATA_T *input,
                               __global DATA_T *output,
                               unsigned int channel,
                               unsigned int height,
                               unsigned int width) {
