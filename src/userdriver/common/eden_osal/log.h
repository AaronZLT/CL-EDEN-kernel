// TODO(jungho7.kim, TBD): This EDEN LOG system will be removed
 ...
 * comment out to mask
 */
static int glive_log = EDEN_LOG_FORCE \
    | EDEN_NN \
    | EDEN_RT \
    | EDEN_UD\
    | EDEN_LINK \
...
#define __LOGX(flag, log_level, fmt, ...)                                                                      \
    if (IS_LOG_ON(flag)) {                                                                                     \
        fprintf(stdout, "[Exynos][EDEN][" EDEN_VERSION "][" LOG_TAG "][%c] %s:" __STR(__LINE__) ": " fmt "\n", \
...
#define __LOGX(flag, log_level, fmt, ...)                                                               \
    if (IS_LOG_ON(flag)) {                                                                              \
        __android_log_print(log_level, "EDEN", "[Exynos][EDEN][" EDEN_VERSION "][" LOG_TAG              \
